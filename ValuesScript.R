#
# Values Script for the London Transport Infrastructure Analysis Platform (LTIAP)
#
...
WardData <- fread('https://raw.githubusercontent.com/wa721/LondonData/master/LondonData.csv')
...
TubeStations <- fread('https://raw.githubusercontent.com/wa721/GIS/master/StationsDF.csv')
...
PopAdjustFactor <- function (variable){
  adj <- sum(TubeStations$variable)/sum(WardData$variable)
  TubeStations$variable/adj
}
  
TubeStations$Popu <- PopAdjustFactor(Popu)
TubeStations$EmployedPeople <- PopAdjustFactor(EmployedPeople)
TubeStations$WorkingAge <- PopAdjustFactor(WorkingAge)
...
if (!(is.na(Outliers(TubeStations$DeprivationRank))[1])){
  ...
  DeprivationRankStations <- TubeStations
}
...
mBikeUse <- leaflet(TubeStations) %>%
  ...
  addLegend("bottomright", pal = palBikeUse, values = TubeStations$BikeUse,
            opacity = 1)%>%
  addScaleBar(position = "bottomleft")%>%
  addControl(img(src="northSign.jpg", width = "5%", height = "5%"),position = "topright")
